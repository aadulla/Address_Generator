...
"""
"Data_Block" represents a block of data inside a MiniMemory instance that is stale
"""
class Data_Block:
    """
    ...
    """
    "is_intersecting_forward" determines whether the start of the Data_Block points to the same
    ...
    def is_intersecting_forward(self, ptr):
        ...
        if self.size != 0:
            # check if ptr points to the same place as the start of the Data_Block
            if ptr == self.start:
    ...
    """
    "is_intersecting_backward" determines whether the end of the Data_Block points to the same
    location/index in the actual memory of a passed in pointer
    params: 
        ptr: a pointer to the location in the actual memory 
    """
    ...
    """
    "increase_forward" expands the Data_Block in the forward direction (i.e. it grows towards the right)
    params: None
    """
    ...
    """
    "reduce_backward" shrinks the Data_Block in the forward direction (i.e. it shirnks towards the right)
    params: None
    """  
    ...
    """
    "adjust_forward" adjusts the Data_Block in the forward direction as a result of the previous
    MiniMemory encroaching on the Data_Block
    params: None
    """
    def adjust_forward(self):
        # if the Data_Block is empty, shift it to the right
        if self.size == 0:
            self.shift_right()
            self.should_increase = False
        # if the Data_Block is non-empty, shrink the Data_Block from the start
        elif self.size != 0:
            self.reduce_forward()
            self.should_increase = True
    ...
    def adjust_backward(self):
        # if the Data_Block is empty, shift it to the left
        if self.size == 0:
            self.shift_left()
            self.should_increase = False
        # if the Data_Block is non-empty, shrink the Data_Block from the end
        elif self.size != 0:
